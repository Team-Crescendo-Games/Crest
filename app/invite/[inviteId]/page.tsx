...
import { AcceptInviteButton } from "@/components/team/accept-button";
    ...
    <div className="flex min-h-screen flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-lg border border-border bg-bg-elevated/80 p-8 backdrop-blur-sm">
        <div className="text-center">
          <h1 className="font-mono text-lg font-semibold text-fg-primary">Workspace Invitation</h1>
          <p className="mt-2 text-xs text-fg-muted">You&apos;ve been invited to join</p>
          <p className="mt-1 font-mono text-sm font-medium text-accent">{invitation.workspace.name}</p>
          {invitation.workspace.description && (
            <p className="mt-1 text-xs text-fg-muted">{invitation.workspace.description}</p>
          )}
          <p className="mt-2 text-[10px] text-fg-muted">Invited by {invitation.createdBy.name}</p>
        </div>
