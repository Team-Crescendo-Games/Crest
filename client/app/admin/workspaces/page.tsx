  ...
  return (
    <div className="group flex items-center gap-1.5">
      ...
      {hasValue && (
        <button
          onClick={handleCopy}
          className="text-gray-400 opacity-0 transition-opacity hover:text-gray-600 group-hover:opacity-100 dark:hover:text-gray-300"
          title="Copy"
        >
          {copied ? (
                        ...
                        <td className="px-4 py-3">
                          <select
                            ...
                            onChange={(e) =>
                              setEditForm({
                                ...editForm,
                                joinPolicy: Number(e.target.value),
                              })
                            }
                            className={inputClass}
                          >
                      ...
                      <>
                          ...
                          {ws.createdBy ? (
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-gray-500">
                                {ws.createdBy.email || `ID: ${ws.createdById}`}
                              </span>
                            </div>
                          ) : (
                        ...
                        <td className="px-4 py-3">
                          <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-600 dark:bg-dark-tertiary dark:text-gray-400">
                            {JOIN_POLICY_LABELS[ws.joinPolicy ?? 0] ??
                              "Unknown"}
                          </span>
                        </td>
                        <td className="px-4 py-3">
        ...
        {workspaces && workspaces.length > pageSize && (
          <div className="flex items-center justify-between border-t border-gray-200 px-4 py-3 dark:border-gray-700">
            <span className="text-sm text-gray-500 dark:text-gray-400">
              ...
              {Math.min((page + 1) * pageSize, workspaces.length)} of{" "}
              {workspaces.length}
            </span>
            <div className="flex gap-2">
              <button
              ...
              <button
                onClick={() =>
                  setPage((p) =>
                    (p + 1) * pageSize < workspaces.length ? p + 1 : p,
                  )
                }
