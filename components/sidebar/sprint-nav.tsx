...
export function SprintNav({ sprints, activeWorkspaceId, pathname }: SprintNavProps) {
  const [expanded, setExpanded] = useState(true);
  const [showArchived, setShowArchived] = useState(false);

  const sprintsActive = pathname.startsWith(`/w/${activeWorkspaceId}/s`);
  const childSprintActive = sprints.some((s) => pathname.startsWith(`/w/${activeWorkspaceId}/s/${s.id}`));

  return (
    <div>
      <div
        className={`flex items-center rounded-md transition-colors ${
          sprintsActive ? (childSprintActive ? "bg-accent/5" : "bg-accent/10") : "hover:bg-bg-secondary"
        }`}
      >
        <button
        ...
        <Link
          ...
          className={`flex flex-1 items-center gap-2 px-2 py-1.5 text-xs font-medium transition-colors ${
            sprintsActive ? "text-accent" : "text-fg-secondary hover:text-fg-primary"
          }`}
        >
          <Timer size={13} />
          Sprints
