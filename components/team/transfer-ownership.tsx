...
export function TransferOwnership({ workspaceId, members }: { workspaceId: string; members: Member[] }) {
    ...
    return (
      <div className="rounded-md border border-accent/30 bg-bg-elevated/60 p-4 backdrop-blur-sm">
        <p className="text-xs font-medium text-fg-primary">
          Transfer ownership to <span className="text-accent">{selected?.name ?? selected?.email}</span>?
        </p>
        <p className="mt-1 text-[11px] text-fg-muted">
          You will lose owner privileges and become a regular member. This action can only be reversed by the new owner.
        </p>
        {error && <p className="mt-2 text-[11px] text-accent-emphasis">{error}</p>}
  ...
  return (
    <div className="rounded-md border border-border bg-bg-elevated/60 p-4 backdrop-blur-sm">
      <p className="text-xs font-medium text-fg-primary">Select the new owner</p>
      <div className="mt-3 space-y-1.5">
        {members.map((member) => (
          ...
          >
            ...
            <div>
              <p className="text-xs font-medium text-fg-primary">{member.name}</p>
